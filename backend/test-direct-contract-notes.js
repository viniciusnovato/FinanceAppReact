...
async function testDirectContractNotes() {
  console.log('=== TESTE DIRETO DE CRIAÇÃO DE CONTRATO COM NOVAS NOTAS ===');
  
  try {
    // Inicializar repositórios e service
    ...
    // 2. Criar um novo contrato diretamente via service
    const contractData = {
      client_id: client.id,
      number: `TEST-NOTES-DIRECT-${Date.now()}`,
      value: 10000,
      down_payment: 1000,
    ...
    console.log('\n📝 Criando contrato diretamente via service...');
    
    const newContract = await contractService.createContract(contractData);
    console.log(`✅ Contrato criado: ${newContract.number} (ID: ${newContract.id})`);
